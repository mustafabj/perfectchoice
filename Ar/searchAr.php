        ...
        <div class="pagination">
            ...
            if (isset($_GET['page']) && $_GET['page'] > 1) {
                if ($_GET['page'] > $total_page) {
                    echo "<a href='searchAr.php?search={$_GET['search']}&page=" . ($total_page > 1 ? $total_page - 1 : 1) . "'>&laquo;</a>";
                } else {
                    echo "<a href='searchAr.php?search={$_GET['search']}&page=" . ($_GET['page'] - 1) . "'>&laquo;</a>";
                }
            } else {
                echo "<a class='isDisabled' href='searchAr.php'>&laquo;</a>";
            }
            for ($i = 1; $i <= $total_page; $i++) {
                if (isset($_GET['page'])) {
                    if ($i == $_GET['page']) {
                        echo "<a class='active' href='searchAr.php?search={$_GET['search']}&page=$i '>$i</a>";
                    } else {
                        echo "<a href='searchAr.php?search={$_GET['search']}&page=$i '>$i</a>";
                    }
                } elseif ($i == 1) {
            ...
            if (isset($_GET['page']) && $_GET['page'] < $total_page) {
                echo "<a href='searchAr.php?search={$_GET['search']}&page=" . ($_GET['page'] + 1) . "'>&raquo;</a>";
            } elseif ($total_page > 1) {
                echo "<a href='searchAr.php?search={$_GET['search']}&page=2'>&raquo;</a>";
            } else {
                echo "<a class='isDisabled' href='searchAr.php'>&raquo;</a>";
            }
            ?>
        </div>
    </div>
